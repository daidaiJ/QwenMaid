...
前端和后端实现必须最小化 Tauri 启动时延：

| 层 | 策略 |
|---|---|
| 前端 | 路由级 `React.lazy` 懒加载，首屏只渲染 shell 骨架，图表/编辑器等重型组件按需加载 |
| 后端 | DB 迁移、代理服务器启动、清理任务用 `tokio::spawn` 延迟执行，不阻塞 `setup` |
| IPC | 按需初始化子系统，首次调用时才建立连接/加载数据，避免 mount 阶段批量 IPC |
